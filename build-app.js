...
if (!fs.existsSync(frontendDir) || !fs.existsSync(backendDir)) {
  console.error(
    "❌ Project structure not found. Run this from habits-tracker root directory."
  );
  process.exit(1);
}
...
async function buildApp() {
  ...
  // 2. Build frontend
  const frontendBuilt = runCommand(
    "Building frontend",
    "npm run build",
    frontendDir
  );
  ...
  if (!electronPrepared) {
    console.error(
      "❌ Failed to prepare Electron files. Fix errors and try again."
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (fs.existsSync(buildDir)) {
      ...
      if (exeFiles.length > 0) {
        ...
        console.log("\n📦 Your application has been built successfully!");
        console.log(
          `   Check the output files in: ${path.join(
            "frontend",
            "electron-build"
          )}`
        );
    ...
    console.log(
      "You can still run your application using 'Start Habits Tracker.bat'"
    );
  }
}
