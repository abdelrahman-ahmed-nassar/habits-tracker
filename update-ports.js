...
function updatePortInFile(filePath) {
  try {
    ...
    const updatedContent = content
      .replace(new RegExp(`localhost:${OLD_PORT}`, 'g'), `localhost:${NEW_PORT}`)
      .replace(new RegExp(`PORT: ["']${OLD_PORT}["']`, 'g'), `PORT: "${NEW_PORT}"`)
      .replace(new RegExp(`port ${OLD_PORT}`, 'g'), `port ${NEW_PORT}`)
      .replace(new RegExp(`waitForPort\\(${OLD_PORT}\\)`, 'g'), `waitForPort(${NEW_PORT})`)
      .replace(new RegExp(`PORT=${OLD_PORT}`, 'g'), `PORT=${NEW_PORT}`);
    ...
    console.error(`❌ Error updating ${path.relative(ROOT_DIR, filePath)}: ${error.message}`);
    return false;
  }
}
...
if (fs.existsSync(electronDir)) {
  const electronFiles = fs.readdirSync(electronDir).filter(f => f.endsWith('.js') || f.endsWith('.cjs'));
...
if (fs.existsSync(distElectronDir)) {
  const distElectronFiles = fs.readdirSync(distElectronDir).filter(f => f.endsWith('.js') || f.endsWith('.cjs'));
...
const scriptFiles = fs.readdirSync(scriptDir).filter(f => f.endsWith('.js') || f.endsWith('.bat'));
